...
from schemas.project import ProjectCreate  # Убедитесь, что импорт правильный

project_blueprint = Blueprint('project', __name__)

@project_blueprint.route("/", methods=["POST"])
def create_project():
    project_data = ProjectCreate(**request.form)
    db: Session = next(get_db())
    project_service = ProjectService(db)
    created_project = project_service.create_project(
        title=project_data.title,
        description=project_data.description  # Убедитесь, что используется поле description
    )
